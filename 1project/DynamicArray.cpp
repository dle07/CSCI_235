...
/** 
    ...
    @return true if the removal was successful; false other wise
*/
template <class T>
bool DynamicArray<T>::remove(const T &an_item)
{
    ...
    {   
        ...
        if(item_count_==0){throw "error";}
        for( int i=0; i< item_count_; i++){
            if( items_[i] == an_item){      //if we see a new item, we create a new dynamic array
                items_[i] = items_[item_count_-1];      //we use the algorithm we learned in class to remove the 
                                                        //item found by replacing it with the last item we care about, and decrementing item count
...
{
    if( atCapacity() ){
        T * tempArray =  new T [ current_capacity_ * 2];            //creates a temporary dynamic array, whose size is 2 times current_capacity

        for( int i=0; i< item_count_; i++){           //copies content of array into the temp array, sets current member capacity to 2 times itself
            tempArray[i] = items_[i];
        }
        current_capacity_ *= 2;     //deletes the previous array to prevent leaks, sets items_ to the temp array address.
        delete [] items_;
        items_ = tempArray;


    }else if(item_count_ < (current_capacity_/4) ){         
        T * tempArray =  new T [ current_capacity_ / 2 ];                   //creates new array accordingly and copies,
